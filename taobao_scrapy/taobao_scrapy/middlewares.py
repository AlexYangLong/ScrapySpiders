...
class SeleniumMiddleware():
    ...
    def process_request(self, request, spider):
        self.logger.debug('Chrome Start Crawling')
        page = request.meta.get('page', 1)
        try:
            self.browser.get(request.url)
            if page > 1:
                input_box = self.wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, '#mainsrp-pager div.form> input')))
                input_btn = self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#mainsrp-pager div.form>span.btn.J_Submit')))
                input_box.clear()
